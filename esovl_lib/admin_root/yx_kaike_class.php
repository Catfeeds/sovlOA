...
if (@$_COOKIE["a_class"]>1) {
  ...
  if (isset($_POST["class_pindao"])){
      $sql="insert into yx_class set class_title='".$_POST["class_title"]."',class_pindao='".$_POST["class_pindao"]."',class_con='".$_POST["class_con"]."',pxk_pic='".$_POST["pxk_pic"]."'";
	  $rs=mysql_query($sql,$conn);
	  if ($rs){
      echo"<script>alert('研修信息添加成功');location.href='yx_list_class.php';</script>";
	  }else{
...
<tr>
	...
	<tr>
	  ...
	  <tr>
        ...
        <input type="text" name="class_title" id="pxk_title" value="" maxlength="100" style="width:300px;"/><span id="p_kbmc">*</span></td>
      </tr>
	  <tr>
	   ...
	   <td colspan="-2" bgcolor="#FFFFFF" class="title"><input type="text" name="pxk_pic" value="" style="width:210px;" readonly/>
          <input name="button" type="button" class="go" onClick="window.open('up2.php?formname=pxkkform&editname=pxk_pic&uppath=upload','','status=no,scrollbars=no,top=400,left=500,width=75,height=70')" value="上传图片" />
