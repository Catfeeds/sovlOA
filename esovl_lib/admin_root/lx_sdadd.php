...
if (@$_COOKIE["a_class"]>1) {
  ...
  if (isset($_POST["lx_sdtitle"])){
	 $sql="insert into lxsdnews set lx_sdbcl=".$_POST["lx_sdbcl"].",lx_sdscl='".$_POST["lx_sdscl"]."',lx_sdtitle='".$_POST["lx_sdtitle"]."',lx_sdcon='".$_POST["lx_sdcon"]."',lx_sdbool=".$_POST["lx_sdbool"].",lx_sdsource='".$_POST["lx_sdsource"]."',lx_sdate='".$_POST["lx_sdate"]."'";
	  $rs=mysql_query($sql,$conn);
	  if ($rs){
      echo"<script>alert('新闻添加成功');location.href='lx_sdlist.php';</script>";
	  }else{
		  echo $sql."<br>";
...
<table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#3e70a5" class="tableborder">
  <tr>
    <th height="24" nowrap="nowrap"><div class="mian_right_box_title_01" style="text-align:left;">留学视点新闻新闻：添加，修改介绍新闻相关的内容</div></th>
  </tr>
  <tr>
...
<form name="lxsdform"  method="post" action="">
  <tr>
    <td colspan="2" class="mian_right_box_title_01">【添加留学视点新闻新闻】</td>
	</tr> 
	<tr>
 ...
 <option id="op_bg" value="0">--请选择分类--</option>
	 ...
	 // $selclass=$row["shopxpbe_id"];
	  ...
	  while ($row=mysql_fetch_array($rs,MYSQL_ASSOC)){
      ?>
		  <option value="<?=$row["lb_id"]?>"><?=$row["lb_title"]?></option>
	  <?php
          }}
	  ?>
</select><span id="lxd"></span>
		选择视点小类：
          <select name="lx_sdscl" size="1" id="lx_sdscl">
            <?php //rs.open "select * from lxsclass where shopxpbe_id="&selclass,conn,1,1
//if not(rs.eof and rs.bof) then
//
//    dim selclass3
// 	selclass3=rs("shopxpse_id")
//do while not rs.eof
//  echo  "<option value="&rs("shopxpse_id")
//  if shopxpse_id=rs("shopxpse_id") then echo " selected "
//  echo ">"&trim(rs("shopxpse_name"))&"</option>"  
//rs.movenext
//loop
//end if
//        rs.close
...
?>
	  ...
	  <tr>
	    ...
	    <td bgcolor="#FFFFFF" class="title"><input type="text" name="lx_sdtitle" maxlength="40" style="width:300px;" onBlur="return lxsdnewsset()"/>
	 ...
	 <!-- <tr>
	  ...
	  <tr>
        ...
        <td bgcolor="#FFFFFF" class="title"><input type="radio" name="lx_sdbool" value="1">
          是 
          <input type="radio" name="lx_sdbool" checked value="0">
          否</td>
    </tr>
	  <tr>
	    <td height="27" align="right" bgcolor="#FFFFFF" class="title">来源：</td>
	    <td bgcolor="#FFFFFF" class="title"><input type="text" name="lx_sdsource" ></td>
      </tr>
	  <tr>
	  ...
	  <tr>
        ...
        <td bgcolor="#FFFFFF" class="title"><input type="submit" name="Submit" onClick="return lxsdset()" value="保存新闻"></td>
    </tr>
  </form>
</table>
</div>
