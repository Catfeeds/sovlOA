...
if ($mid==""){
  ...
  if (isset($_POST["ntitle"])){ 
      ...
      $sql="update ennews set ntitle='".$_POST["ntitle"]."',smclass='".$_POST["smclass"]."',npic='".$_POST["npic"]."',nclass=".$_POST["nclass"].",ncon='".$_POST["ncon"]."',nbool=".$_POST["nbool"]." where nid=".$_POST["nid"];
    ...
    <form name="formnews" method="post" onSubmit="return newsset()" action="">
     ...
     <?php 
      ...
      ?>
        ...
        <span id="ncl">*</span> <span class="title" style="padding-left:12px;">新闻小类
        <select name="smclass">
      <?php 
      $sqb="select * from pxsclass";
	  $rsb=mysql_query($sqb,$conn);
	  if (mysql_num_rows($rsb)>=1){
	  while ($rowb=mysql_fetch_array($rsb,MYSQL_ASSOC)){
      ?>
          <option value="<?php echo $rowb["ps_id"]?>" <?php if($rowb["ps_id"]==$row["smclass"]){echo "selected";}?>><?php echo $rowb["ps_title"]?></option>
          <?php
          }
		  }?>
        </select>
