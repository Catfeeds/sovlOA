...
if (@$_GET["del"]=="ok"){
	...
	$rs=mysql_query($sql);
	if ($rs) echo"<script>alert('删除成功');location.href='yx_kaoshi_class.php';</script>";
...
if (isset($_POST["kcedit"])){//频道值是数值型
      $sql="update yx_kaoshi_class set class_name='".$_POST["class_name"]."' where class_id=".$_POST["class_id"];
	//exit($sql);
	 $rs=mysql_query($sql,$conn);
	  if ($rs){
	 
      echo"<script>alert('更新成功');location.href='yx_kaoshi_class.php';</script>";	  
	  }else{
  ...
  if (isset($_POST["zyadd"])){
      $sql="insert into yx_kaoshi_class set class_name='".$_POST["class_name"]."'";
	 //exit($sql);
	  $rs=mysql_query($sql,$conn);
	  if ($rs){	 
      echo"<script>alert('添加成功');location.href='yx_kaoshi_class.php';</script>";
	  }else{	  
