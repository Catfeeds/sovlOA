...
if (@$_COOKIE["a_class"]>1) {
  ...
  if (isset($_POST["lq_name"])){ 
      $sql="update luqu set s_id=".$_POST["s_id"].", lq_name='".$_POST["lq_name"]."' where lq_id=".$_POST["lq_id"];

	  $rs=mysql_query($sql,$conn);
	  if ($rs){
	
      echo"<script>alert('修改成功');location.href='lqlist.php?id=".$_POST["cid"]."';</script>";	  
	  }else{
      ...
      $sql="select * from luqu where lq_id=".$_GET["id"];
	  $rs=mysql_query($sql,$conn);
	  if (!$rs){	 
  ...
  <table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#93B3E9">
	...
	<select name="s_id">
	  ...
	  <td colspan="-2" bgcolor="#FFFFFF" class="title" style="padding-left:12px;">
	    <select name="lq_zy">
          <?php 
      $sql0="select * from xlmc";
	  $rs0=mysql_query($sql0,$conn);
	  if (mysql_num_rows($rs0)>=1){
	  while ($row0=mysql_fetch_array($rs0,MYSQL_ASSOC)){
      ?>
          <option value="<?php echo $row0["mc_title"]?>" <?php if ($row["lq_zy"]==$row0["mc_title"]){echo "selected";}?>><?php echo $row0["mc_title"]?></option>
          <?php
          }
		  }?>
        </select>
