...
$rowk = $dblink->find('lxsclass','',"ls_id=".$_GET["sid"]);
$stitle=$rowk["ls_title"];	

$web['z_title'] = $stitle.'--留学频道--'.$z_name;
			...
			<div class="main_news_post">
				<div class="main_news_post_box">
					<div class="main_news_post_box_title">
						...
						<a href="http://<?php echo $_SERVER['SERVER_NAME'];?>/"><?=$z_name?></a> >> <a href="/Abroad/">留学频道</a> >> <?=$stitle?>
						</dd>
						</dl>
					</div>
...
$url=parse_url($url);
@$url=$url[path];
$num = $dblink->countByStr('lxsdnews','',"lx_sdscl='".$_GET["sid"]."'");
...
$page.=',';
      ...
      @$sql="SELECT * FROM lxsdnews where lx_sdscl='".$_GET["sid"]."' order by lx_sdid  desc limit $page $pagesize ";
...
?>
						...
						<div class="main_news_post_box_list00">
							...
							<dl>
							<dt><a href="lx_sdnews_detail.php?id=<?=$row["lx_sdid"]?>" title="<?=$row["lx_sdtitle"]?>"><?=$row["lx_sdtitle"]?></a></dt>
							<dd><?=date("Y-m-d", strtotime($row["lx_sdate"]));?></dd>
							</dl>
							</div>
				<div class="main_news_post_box_list00_l"><?=$Common->strCut(strip_tags($row["lx_sdcon"]),280)?>..  <a href="lx_sdnews_detail.php?id=<?=$row["lx_sdid"]?>">查看详细>></a>
							</div>
						</div>
...
echo" <li><a href=$url?sid=".$_GET["sid"]."&page=".($pageval-1)." class='width48'>上一页</a></li>";
}
    for ($i=1;$i<=$cp;$i++){
	     if ($i==@$_GET["page"]){
		 echo "<li><a href='#' class='fy_bg'>".$i."</a></li>";
		 }else{
	     echo "  <li><a href=$url?sid=".$_GET["sid"]."&page=".$i.">".$i."</a></li>  ";
		 }
	}
if(@$_GET["page"]<$cp){
echo " <li><a href=$url?sid=".$_GET["sid"]."&page=".($pageval+1)." class='width48'>下一页</a></li>";
}
} 
?>
						</ul>
