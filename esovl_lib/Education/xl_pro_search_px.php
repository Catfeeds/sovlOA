...
if (@$_GET["xfei"]=="asc"){
@$sql="select * from kkinfo  order by xfei asc limit $page $pagesize ";
}elseif (@$_GET["xfei"]=="desc"){
@$sql="select * from kkinfo  order by xfei desc limit $page $pagesize ";
}elseif (@$_GET["xfei"]=="20000"){
@$sql="select * from kkinfo where xfei<20000 limit $page $pagesize ";
}elseif(@$_GET["xfei"]=="20000-25000"){
@$sql="select * from kkinfo where xfei between 20000 and 25000 limit $page $pagesize ";
}elseif(@$_GET["xfei"]=="25000"){
@$sql="select * from kkinfo where xfei>25000 limit $page $pagesize ";
}else{
@$sql="select * from kkinfo limit $page $pagesize ";
}

if (@$_GET["xfei"]!=""){

if (@$_GET["xfei"]=="asc"){
$numq=mysql_query("SELECT * FROM kkinfo order by xfei asc");
}elseif (@$_GET["xfei"]=="desc"){
$numq=mysql_query("SELECT * FROM kkinfo order by xfei desc");
}elseif (@$_GET["xfei"]=="20000"){
$numq=mysql_query("SELECT * FROM kkinfo where xfei<20000");
}elseif(@$_GET["xfei"]=="20000-25000"){
$numq=mysql_query("select * from kkinfo where xfei between 20000 and 25000");
}elseif(@$_GET["xfei"]=="25000"){
$numq=mysql_query("SELECT * FROM kkinfo where xfei>25000");
}
...
echo" <a href=$url?xfei=".@$_GET["xfei"]."&page=".($pageval-1)."><<</a>";
}else{
echo" <a href=$url?xfei=".@$_GET["xfei"]."&page=1><<</a>";
}
    for ($i=1;$i<=$cp;$i++){
	     if ($i==@$_GET["page"]){
		 echo " <span class='bwhite'>".$i."</span> ";
		 }else{
	     echo " <a href=$url?xfei=".@$_GET["xfei"]."&page=".$i.">".$i."</a> ";
		 }
	}
if(@$_GET["page"]<$cp){
echo " <a href=$url?xfei=".@$_GET["xfei"]."&page=".($pageval+1).">>></a>";
}else{
echo " <a href=$url?xfei=".@$_GET["xfei"]."&page=".$cp.">>></a>";
}
...
<label>转到</label>
			...
			if ($i==@$_GET["page"]){
			echo "<option value='$url?xfei=".@$_GET["xfei"]."&page=".$i."' selected>".$i."</option>";
			}else{
			echo "<option value='$url?xfei=".@$_GET["xfei"]."&page=".$i."'>".$i."</option>";
			}
             }
			?>		
			</select>
