 <?php
		...
		if (mysql_num_rows($rs)>=1){	
		   ...
		   $bx_title=$row["z_title"];// 网站标题
		   $bx_keyword=$row["z_keyword"];// 网站优化关键词
		   $bx_contant=$row["z_contant"];// 网站优化描述
         
		   $bx_bmtj=$row["z_bmtj"];//报名条件
		   $bx_bmtel=$row["z_tel"];//报名电话
		   $xl_banner=$row["z_banner"];//动画Banner//每个页调用的参数是一个，所以命名一样的。
		   $bx_onegg=$row["z_onegg"];//首页中间广告图
		   $bx_gglink=$row["z_gglink"];//广告链接
