...
?>
<?php
       $rs1=mysql_query("select n_class from enclass where n_id=".$_GET["cid"]);
...
$url=parse_url($url);
@$url=$url[path];
$numq=mysql_query("SELECT * FROM ennews where nclass=".$_GET["cid"]);
...
$page.=',';
      ...
      @$sql="select * from ennews where nclass=".$_GET["cid"]." order by ndate desc limit $page $pagesize ";     
	  $rs=mysql_query($sql,$conn);
	  if (!$rs){  
	  exit("数据库操作失败! 错误信息为:".mysql_error());
	  }
     ...
     $i=$i+1;	 
		...
		if($i==1&&@$page<2){	 
?>
            <div class="zx_xw_dk_003">
              <div class="zx_xw_tu"><a href="<?=$z_website?>Education/gfb/gf_zx_neiye.php?id=<?=$row["nid"]?>"><img src="../../admin_root/<?=$row["npic"]?>" /></a></div>
              <div class="zx_xw_bt_004"><a href="<?=$z_website?>Education/gfb/gf_zx_neiye.php?id=<?=$row["nid"]?>"><?=$row["ntitle"]?></a></div>
              <div class="zx_xw_sj" style="float:right;width:70px;"><?=date('Y-m-d',strtotime($row["ndate"]))?></div>
              <div class="zx_xw_nr_005"><?=utf_substr($row["ncon"],250)?>..</div>
            </div>
             <?php }else{?>
            <div class="zx_xw_dk_006">
              <div class="zx_wzgg_bt_007"><span><a href="<?=$z_website?>Education/gfb/gf_zx_neiye.php?id=<?=$row["nid"]?>"><?=$row["ntitle"]?></a></span>2010-12-03</div>
              <div class="zx_wzgg_nr_008"><?=utf_substr($row["ncon"],296)?>..<span><a href="<?=$z_website?>Education/gfb/gf_zx_neiye.php?id=<?=$row["nid"]?>">查看详细>></a></span></div>
            </div>
            <?php }}}?>
         
            <div class="zx_gg_fy_009"> <span style="display:block;float:left;"><?php echo "共 $num 条信息";?></span>
...
echo" <div class='zx_fy_syy'><a href=$url?cid=".$_GET["cid"]."&page=".($pageval-1)."><img src='../../images/wl-gg_18.jpg' /></a></div>";
}
    for ($i=1;$i<=$cp;$i++){
	     if ($i==@$_GET["page"]){
		 echo " <div class='zx_fy_sz'>".$i."</div> ";
		 }else{
	     echo " <div class='zx_fy_sz'><a href=$url?cid=".$_GET["cid"]."&page=".$i.">".$i."</a></div> ";
		 }
	}
if(@$_GET["page"]<$cp){
echo " <div class='zx_fy_syy'><a href=$url?cid=".$_GET["cid"]."&page=".($pageval+1)."><img src='../../images/wl-gg_28.jpg' /></a></div>";
}
