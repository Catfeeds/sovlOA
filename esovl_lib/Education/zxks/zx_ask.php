...
for(var i=1;i<=3;i++){g('tb_'+i).className='normaltab1';g('tbc_0'+i).className='undis';}g('tbc_0'+n).className='dis';g('tb_'+n).className='hovertab1';
              ...
              <div class="zx_zx_dk">
                  <div class="zx_zx_fd">
                    <div class="zx_zx_lj">
                    	<dl>
                          <dd><span><a href="#">首页</a></span></dd><dt><img src="../../images/bx_hw_03.jpg" width="17" height="30" /></dt>
                          <dd><a href="Education/zxks/">自学考试</a></dd><dt><img src="../../images/bx_hw_03.jpg" width="17" height="30" /></dt>
                          <dd><strong><?=$ction?></strong></dd><dt><img src="../../images/bx_hw_03.jpg" width="17" height="30" /></dt>
                        </dl>
                    </div>
                  </div>
                  <div class="zx_wzgg">
...
$numq=mysql_query("SELECT * FROM wdonline where s_name in(".$sname.") and wd_bool=1");
...
$page.=',';
      ...
      @$sql="select * from wdonline where s_name in(".$sname.") and wd_bool=1 order by wd_stime desc limit $page $pagesize "; 
	  $rs=mysql_query($sql,$conn);
	  if (!$rs){  
	  exit("数据库操作失败! 错误信息为:".mysql_error());
	  }
	 ...
	 if (mysql_num_rows($rs)>=1){	 
	 while ($row=mysql_fetch_array($rs,MYSQL_ASSOC)){
    	$rsa=mysql_query("select s_id,s_name from school where s_name='".$row["s_name"]."'",$conn); 
		 $rowa=mysql_fetch_assoc($rsa);
		   $rsb=mysql_query("select * from kkinfo where s_id=".$rowa["s_id"],$conn); 
		   $rowb=mysql_fetch_assoc($rsb);
		 // if ($rowb["bk_id"]==1){
			
?>                   
                     
           <div class="zx_xw_dk2">
           <div class="zx_wzgg_bt"><span><a href="Education/xl_pro_detail.php?kid=<?=$rowb["kid"]?>&sid=<?=$rowb["s_id"]?>"><?=$rowa["s_name"]?></a></span>提问时间：<?=date('Y-m-d',strtotime($row["wd_stime"]))?></div>
           <div class="zx_wzgg_nr">问：<?=$row["wd_ask"]?></div>
           <div class="zx_wzgg_nr">答：<?=$row["wd_answer"]?></div>
           </div>
