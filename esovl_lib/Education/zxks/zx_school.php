...
$url=parse_url($url);
@$url=$url[path];
$numq=mysql_query("SELECT * FROM kkinfo where bk_id=1");
...
$page.=',';
      ...
      @$sql="select * from kkinfo where bk_id=1 order by kkdate desc limit $page $pagesize ";
	  $rs=mysql_query($sql,$conn);
	  if (!$rs){  
	  exit("数据库操作失败! 错误信息为:".mysql_error());
	  }
...
?>                
           <div class="zx_xw_dk2">
           <div class="zx_wzgg_bt"><span><a href="Education/xl_pro_detail.php?kid=<?=$row["kid"]?>&sid=<?=$row["s_id"]?>"><?=$rowa["s_name"]?></a>(<?=$row["zyclass"]?>)</span><?=date('Y-m-d',strtotime($row["kkdate"]))?></div>
           <div class="zx_wzgg_nr"><?=utf_substr($rowa["s_xyjs"],280)?>.. <span><a href="Education/xl_pro_detail.php?kid=<?=$row["kid"]?>&sid=<?=$row["s_id"]?>">查看详细>></a></span></div>
           </div>
