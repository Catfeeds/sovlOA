    ...
    <div class="px_pd">
		<div class="px_pd_box01">
            ...
            <div class="px_pd_box01_nav" id="px_n02">
				<?php
				//根据频道号查出BID号再查小类别
				$models = Icolumn::model()->getAllByPid(18);
				foreach($models as $v){
		...
		<div class="px_pd_box03">
			...
			<div class="px_pd_box03_center" id="px_pd_box03_center_02">
				...
				<?php
					$model = Information::model()->getAllByid(74,'1','i_submitdate desc');
					if($model != ''){
						foreach($model as $row){
				?>
						<dl>
							<dt id="new02"><a href="<?=Yii::app()->createUrl("/training/newsview",array("id"=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=$row["i_title"]?></a></dt>
				...
				<div class="px_pd_box03_center_box02">
					<ul>
						<?php
							...
							foreach($model as $row){?>
								<li><a href="<?=Yii::app()->createUrl("/training/newsview",array("id"=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=Common::strCutAndTags($row["i_title"],40)?></a></li>
					<?php	}?>  
					</ul>
				</div>
			</div>
	...
	<div class="mian_px_index_news">
            ...
            <div class="mian_px_index_ky_bk_02">
				...
				<?php
					...
					foreach($psModel as $sql){
						...
						foreach($res2 as $row){
				?>
                        <dd style="width:120px;float:left;">[<a href="px_school/?pid=<?=$row["pxk_sid"]?>"><?php if($row["pxk_sid"]==$psid){echo Common::strCut($sql['pxs_name'],18);}?></a>]</dd><dt style=" float:left;width:110px;" title="<?=$row["pxk_title"]?>"><a href="<?=Yii::app()->createUrl("/training/pxkcview",array("id"=>$row['pxk_id']))?>"><?=Common::strCut($row['pxk_title'],16)?></a></dt><dt style=" float:left;width:57px; color:#f00;"><?=$row["pxk_yhui"]?></dt>
                        <?php }}?>
                    </dl>
                </div>
            </div>
		...
		<div class="mian_px_index_right">
            ...
            <div class="mian_px_index_wyxw_bk_02" style="height:auto;">
            	<div class="mian_px_index_wyxw_bj_02">
                	<div class="mian_px_index_ywxw_bt_02"><span>在线问答</span><a href="<?=Yii::app()->createUrl("/training/zxwdlist")?>">更多...</a></div>
                </div>
