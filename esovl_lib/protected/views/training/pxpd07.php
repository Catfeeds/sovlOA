    ...
    <div class="px_pd">
		<div class="px_pd_box01">
            ...
            <div class="px_pd_box01_nav" id="px_n07">
                <?php
				//根据频道号查出BID号再查小类别
				$models = Icolumn::model()->getAllByPid(16);
					foreach($models as $v){
		...
		<div class="px_pd_box03">
			...
			<div class="px_pd_box03_center" id="px_pd_box03_center_01">
				<div class="px_pd_box03_center_box01">
					<?php
						$models = Information::model()->getAllByid(72,'1','i_submitdate desc');
						if($models != ''){
				...
				<div class="px_pd_box03_center_box02">
					<ul>
						<?php
							$models = Information::model()->getAllByid(72,'10','i_submitdate desc');
							foreach($models as $row){
						?>
						<li><a href="<?=Yii::app()->createUrl("/training/newsview",array("id"=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=Common::strCut($row['i_title'],35)?></a></li>
						<?php }?>  
                    ...
                    <dl>
						<?php	
							...
							foreach($psModel as $sql){
								...
								foreach($res2 as $row){
						?>
							<dd style="width:120px;float:left;">[<a href="px_school/?pid=<?=$row["pxk_sid"]?>"><?=$Common->strCut($row["pxs_name"],18)?></a>]</dd><dt style=" float:left;width:110px;" title="<?=$row["pxk_title"]?>"><a href="<?=Yii::app()->createUrl("/training/pxkcview",array("id"=>$row['pxk_id']))?>"><?=$Common->strCut($row["pxk_title"],16)?></a></dt><dt style=" float:left;width:57px; color:#f00;"><?=$row["pxk_yhui"]?></dt>
                        <?php }}?>
                    </dl>
                </div>
        	...
        	<div class="mian_px_index_wyxw_bk">
                ...
                <div class="mian_px_ywxw_news_lb">
                	<ul>
						<?/**/?>
						<?php
							// $res = $dblink->findAll('ennews','','nclass in(69)','','1,10');
							$models = Icolumn::model()->getAllByPid(16);
