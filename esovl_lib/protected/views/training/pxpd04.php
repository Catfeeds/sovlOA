...
<div class="px_pd">
	<div class="px_pd_box01">
		<div class="px_pd_box01_title" id="px_t04"><?=$pdao?></div>
		<div class="px_pd_box01_nav" id="px_n04">
			<?php
				//根据频道号查出BID号再查小类别	
				$models = Icolumn::model()->getAllByPid(15);
				foreach($models as $v){
					$res = Icolumn::model()->getAllTagsByid($v);
					foreach($res as $row1){				
			?>
			<a href="<?=Yii::app()->createUrl("/training/pxkclist",array("id"=>$row1['ic_id']))?>"><?=$row1["ic_class"]?></a><span>|</span> 
			<?php }}?>
		</div>
	</div>
	<div class="px_pd_box03">
		...
		<div class="px_pd_box03_center" id="px_pd_box03_center_01">
			<div class="px_pd_box03_center_box01">
				<?php
					$model = Information::model()->getAllByid(71,'1','i_submitdate desc');
					if($model != ''){
						foreach($model as $row){
				?>
				<dl>
					<dt id="new01"><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=$row["i_title"]?></a></dt>
					<dd><?=Common::strCutAndTags($row["i_con"],220)?><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>" title="<?=$row["i_title"]?>" title="<?=$row["i_title"]?>">[详情]</a></dd>
				</dl>
				<?php }}?>
			</div>
			<div class="px_pd_box03_center_box02">
				<ul>
					<?php
						$model = Information::model()->getAllByid(74,'10','i_submitdate desc');
						foreach($model as $row){
					?>
			...
			<div class="mian_px_index_ky_news_bt">
				<ul>
					<?php
						...
						foreach($model as $row){
					?>
						<li style="line-height:25px;"><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?php echo Common::strCut($row["i_title"],24);?></a></li>
					<?php }?>
				</ul>
			</div>
				...
				<dl>	<?/**/?>
					<?php
						...
						foreach($psModel as $sql){
							...
							foreach($res2 as $row){
					?>
						<dd style="width:120px;float:left;">[<a href="px_school/?pid=<?=$row["pxs_id"]?>"><?php if($row["pxk_sid"]==$psid){echo Common::strCut($sql['pxs_name'],18);}?></a>]</dd><dt style=" float:left;width:110px;" title="<?=$row["pxk_title"]?>"><a href="<?=Yii::app()->createUrl("/training/pxkcview",array("id"=>$row['pxk_id']))?>"><?=Common::strCut($row['pxk_title'],16)?></a></dt><dt style=" float:left;width:57px; color:#f00;"><?=$row["pxk_yhui"]?></dt>
					<?php }}?>
				</dl>
			</div>
		</div>
		...
		<div class="mian_px_index_wyxw_bk">
			...
			<div class="mian_px_ywxw_news_lb">
				<ul>
					<?/**/?>
					<?php
						$res = Information::model()->getAllByid(71,'3','i_submitdate desc');
						foreach($res as $row){
					?>
							<li style="line-height:25px;"><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=Common::strCut($row['i_title'],38)?></a><span><?=$row["i_submitdate"]?date('Y-m-d ',$row["i_submitdate"]):"";?></span></li>
					<?php }?> 
