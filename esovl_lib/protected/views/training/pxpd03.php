...
?>	
    <div class="px_pd">
		<div class="px_pd_box01">
        	<div class="px_pd_box01_title" id="px_t07"><?=$pdao?></div>
            <div class="px_pd_box01_nav" id="px_n07">
                <?php
					//根据频道号查出BID号再查小类别
					$models = Icolumn::model()->getAllByPid(17);
					foreach($models as $v){
		...
		<div class="px_pd_box03">
			...
			<div class="px_pd_box03_center" id="px_pd_box03_center_01">
				<div class="px_pd_box03_center_box01">
					<?php
						$models = Information::model()->getAllByid(73,'1','i_submitdate desc');
						if($models != ''){
							foreach($models as $row){	
					?>
					<dl>
						<dt id="new01"><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>"><?=$row["i_title"]?></a></dt>
				...
				<div class="px_pd_box03_center_box02">
					<ul>
						<?php
							$res = Information::model()->getAllByid(73,'10','i_submitdate desc');
							foreach($res as $row){?>
                    ...
                    <dl>
						<?php
							...
							foreach($psModel as $sql){
								$psid = $sql['pxs_id'];$row1_id = $row1['ic_id'];
