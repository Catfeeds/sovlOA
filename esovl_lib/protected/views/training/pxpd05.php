...
<div class="px_pd">
	<div class="px_pd_box01">
		<div class="px_pd_box01_title" id="px_t05"><?=$pdao?></div>
		<div class="px_pd_box01_nav" id="px_n05">
			<?php
			//根据频道号查出BID号再查小类别
			$models = Icolumn::model()->getAllByPid(14);
				foreach($models as $v){
	...
	<div class="px_pd_box03">
		...
		<div class="px_pd_box03_center" id="px_pd_box03_center_01">
			<div class="px_pd_box03_center_box01">
				<?php
					$models = Information::model()->getAllByid(70,'1','i_submitdate desc');
					if($models != ''){
				...
				<ul>
					<?php
						...
						foreach($models as $v){
							...
							foreach($res as $row){
					?>
						<li><a href="<?=Yii::app()->createUrl("/training/newsview",array('id'=>$row['i_id']))?>" title="<?=$row["i_title"]?>"><?=Common::strCut($row['i_title'],38)?></a></li>
					<?php }}?>  	
