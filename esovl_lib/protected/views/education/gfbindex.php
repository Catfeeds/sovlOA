    ...
    <div class="main_xllb">
        <div class="gf_xx_kuang">
			<div class="gf_ff_left">
				...
				<?php	$models=Information::model()->getAllByLable(array("51"),6,11);
						foreach($models as $i=>$row){
							if($i>0)break;?>
							<div class="gf_ff_bt">
								<a target="_blank" href="<?=Yii::app()->createUrl("education/newsview",array('id'=>$row["i_id"]))?>" title="<?=$row["i_title"]?>">
									<?=Common::strCut($row["i_title"],38)?>
								</a>
							</div>
							<div class="gf_nr">
								<?=Common::strCutAndTags($row["i_con"],124)?>
								<span>
				...
				<div class="gf_ff_rz">
					...
					<?php	foreach($models as $i=>$row){
								if($i<1)continue;?>
								<li><span>·<a  target="_blank" href="<?=Yii::app()->createUrl("education/newsview",array('id'=>$row["i_id"]))?>" title="<?php echo $row["i_title"];?>"><?php echo Common::strCut($row["i_title"],70)?></a></span><?=date('Y-m-d',strtotime($row["i_submitdate"]))?></li>
					<?php	}?>
					</ul>
				</div>
			</div>
			<div class="gf_ff_right">
				<div class="gf_ks">
					<div class="gf_ks_lm">
						<span>考试信息</span>
						<a  target="_blank" href="<?=Yii::app()->createUrl("education/newslist",array('id'=>50))?>">更多</a>...
					</div>
				</div>
				<div class="gf_ks_xian"></div>
				<div class="gf_ks_bt">
					<ul>
					<?php	$models=Information::model()->getAllByLable(array("50"),7,11);
							foreach ($models as $row){?>
								<li>
									<a  target="_blank" href="<?=Yii::app()->createUrl("education/newsview",array('id'=>$row["i_id"]))?>" title="<?=$row["i_title"]?>">
									<?=Common::strCut($row["i_title"],30)?>
									</a>
								</li>
					<?php	}?>
					</ul>
							...
							foreach($listArr as $key=>$value){?>
								<div class="zx_qr_01_1<?=$key+2?>" style="width:617px;">
									<div class="zx_qr_02_left_list">
										...
										<?php	$kkinfomodels=Kkinfo::model()->getAllByZyclass($value,6);
												foreach ($kkinfomodels as $k=>$row1){?>
													<div class="zx_qr_02_left_list_02_004" id="Marquee">
														<dl>
															...
															<dd>
																<span>
																	<a target="_blank" href="<?=Yii::app()->createUrl("education/school",array("id"=>$row1["s_id"],"type"=>"bm","kid"=>$row1["kid"]))?>">
																		报名
																	</a>
																</span>
															</dd>
