        ...
        <div class="main_xllb_box01">
						...
						<div class="main_xllb_box01_center_list02_box2">
							<div class="main_xllb_box01_center_list02_box2_list">
								...
								<div class="xl_undis" id="xl_tbc_04">
									<div class="xl_gfb_hd">
									<?php 	$news=Information::model()->getAllByLable(array('50'),6,11);;
											foreach ($news as $k=>$row){
												if($k>0)break;?>
												<div class="gf_ff_bt">
													<a href="<?=Yii::app()->createUrl('education/newsview',array("id"=>$row->i_id))?>" title="<?php echo $row["i_title"];?>">
														<?php echo Common::strCut($row["i_title"],38)?>
													</a>
												</div>
												<div class="gf_nr">
													<?=Common::strCutAndTags($row["i_con"],124)?><span><a href="<?=Yii::app()->createUrl('education/newsview',array("id"=>$row->i_id))?>" title="<?php echo $row["i_title"];?>">[详细]</a></span>
												</div>
									<?php	}?>	
										<div class="gf_ff_rz">
											<ul>
												<?php	foreach ($news as $k=>$row){
															if($k==0)continue;?>
															<li><span>·<a href="<?=Yii::app()->createUrl('education/newsview',array("id"=>$row->i_id))?>" title="<?php echo $row["i_title"];?>"><?php echo Common::strCut($row["i_title"],70)?></a></span>
											...
											<?php	$criteria=new CDbCriteria;
													...
													foreach ($rs1 as $k=>$row1){
														...
														<div class="zx_qr_02_left_list_02_004" id="Marquee">
															<dl>
															  <dt><span>[<a href="<?=Yii::app()->createUrl("education/school",array("id"=>$row1["s_id"],"type"=>"zyjs","kid"=>$row1["kid"]))?>">
																<?=$row1["zyname"]?>
																</a>]<a href="<?=Yii::app()->createUrl("education/school",array("id"=>$row1["s_id"],"type"=>"zyjs","kid"=>$row1["kid"]))?>" title="<?=$row1["ktitle"]?>">
																  <?=$row1["ktitle"]?>
																</a></span></dt>
															  <dd><span>￥<?=$row1["yhui"]?></span></dd>
															  <dd><span><a href="<?=Yii::app()->createUrl("education/school",array("id"=>$row1["s_id"],"type"=>"bm","kid"=>$row1["kid"]))?>">报名</a></span></dd>
															</dl>
														</div>
										  <?php 	}?>
										</div>
							...
							foreach($arr as $key=>$val){?>
								<div class="main_xl_box02_list" <?=$key=="2"?'style="margin-right:0;"':''?>>
									<div class="main_xl_box02_list_title" >
										  <?php	
												$rows=Information::model()->getAllByLable(array($val),5,6);
												//$row=$rows[0];?>
										  <span><?=Icolumn::model()->getNameByid($val)?></span> 
										  <a href="<?=Yii::app()->createUrl('education/newslist',array("id"=>$val))?>">+MORE</a>
									</div>
													...
													<dl>
														<dt>
															<div class="main_xl_box02_list_text_box1_pic">
																<img src="<?=$row["i_pic"]==0||!$row["i_pic"]?"/images/xl_box2_01.jpg":$row["i_pic"]?>" />
															</div>
														</dt>
														<?php	
																?>
														<dd>
															<a title="<?=strip_tags($row["i_con"])?>" href="<?=Yii::app()->createUrl('education/newsview',array("id"=>$row->i_id))?>">
																<?php	echo Common::strCutAndTags($row["i_con"],84);?>
															</a>
														</dd>
													</dl>
										<?php 	}?>
										...
										<div class="main_xl_box02_list_text_box2">
											<?php	foreach ($rows as $k=>$row){
														...
														<dl>
																...
																<dd>
																	<a  title="<?=$row["i_title"]?>" href="<?=Yii::app()->createUrl('education/newsview',array("id"=>$row->i_id))?>">
																		<?php echo trim($row["i_title"]);?>
																	</a>
																</dd>
														</dl>
											<?php 	}?>
			...
			<div class="main_xllb_box01_right">
				...
				<div class="main_xllb_box01_right_list01">
					<div class="main_xllb_box01_right_list01_title">
						<?php	
						$cjwt=Icolumn::model()->getNameByid(64);
						$cjwtNews=Information::model()->getAllByLable(array("64"),4,6);?>
						<span>
							<?php echo Icolumn::model()->getNameByid(64);?>
						</span>
						<a href="<?=Yii::app()->createUrl("education/newslist",array('id'=>64))?>">
