								...
								<?php 	$criteria=new CDbCriteria;
										...
										foreach  ($models as $row1){
											...
											<div class="main_xl_gxjz_left_box_list_box00_list">
												<h1><img src="/images/dot_l_s.jpg" width="8" height="7" /><a href="<?=Yii::app()->createUrl("education/school",array("id"=>$row1->s_id))?>"><font color="#3C82C0"><?php echo $row1->schoolinfo->s_name?></font></a></h1>
												<ul>
												<?php	foreach ($Kkinfos as $kk=>$rowp){?>
															<li>·<a href="<?=Yii::app()->createUrl("education/school",array("id"=>$rowp->s_id,"type"=>"zyjs","kid"=>$rowp->kid))?>" title="<?php echo $rowp["ktitle"];?>">[<?php echo $rowp["zyclass"];?>]<?php echo $rowp["zyname"];?></a></li>
												<?php 	}?>
												</ul>
											</div>
								<?php 	}?>
											...
											<div class="main_xl_gxjz_left_box_list_box00_list">
												...
												<?php	$Kkinfos=Kkinfo::model()->findAll("zyname='{$rowa["zyname"]}' and bk_id='{$row["bk_id"]}'");
														foreach  ($Kkinfos as $rowb){
															...
															<li>·<a href="<?=Yii::app()->createUrl("education/school",array("id"=>$rowb->s_id,"type"=>"zyjs","kid"=>$rowb->kid))?>" title="<?php echo $rowb["ktitle"];?>(<?php echo $rowb["zyclass"];?>)">[<?php echo $rw["s_name"];?>]<?php echo $rowb["ktitle"];?>(<?php echo $rowb["zyclass"];?>)</a></li>
												<?php	}?>
												</ul>
											</div>
								<?php 	}?>
