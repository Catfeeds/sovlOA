            ...
            <div class="main_xl_detail_box03_main_left_box2">
                ...
                <div class="main_xl_detail_box03_main_left_box2_text">
					<div class="main_xl_wsbm">
						...
						$Xlcalmodel=Xlcal::model()->getAllZyListBySid($model->s_id);
						
						$Xlmcmodel=array();
						$Xlmclist=Xlmc::model()->getAllKcList();
						$XlcalSelect="";
						$XlmcSelect="";
						...
						if(isset($_GET['kid'])&&$_GET['kid']){
							...
							foreach($Xlmclist as $key=>$val){
								if($val==$kkmodel['zyname'])$Xlmcmodel[$key]=$val;
							}
							if($kkmodel){
						...
						<div class="main_wsbm_box_left_box_list02">
							...
							<dl>
								<dt>专业类别</dt>
								<dd>
									<?php echo $form->dropDownList($WsbmModel, "bm_zyclass",$Xlcalmodel,array("name"=>"bm_zyclass","empty"=>"--请选择--","onchange"=>"changeNext(this)",'options' => array($XlcalSelect=> array('selected'=>'selected'))));?>
							...
							<dl>
								...
								<dd>
									...
									<label for="bm_name" generated="true" class="error"><?=$WsbmModel->hasErrors('bm_name')?$form->error($WsbmModel,'bm_name'):"<strong>*</strong><span>请输入中文名</span>"?></label>
								</dd>
							</dl>
							...
							<dl>
								...
								<dd>
									...
									<label for="bm_tel" generated="true" class="error"><?=$WsbmModel->hasErrors('bm_tel')?$form->error($WsbmModel,'bm_tel'):"<strong>*</strong><span>请输入手机</span>"?></label>
								</dd>
							</dl>
							...
							<dl>
								...
								<dd>
									...
									<label for="bm_sfz" generated="true" class="error"><?=$WsbmModel->hasErrors('bm_sfz')?$form->error($WsbmModel,'bm_sfz'):"<strong>*</strong><span>用于报名确认，请填写真实身份证号码</span>"?></label>
								</dd>
							</dl>
							...
							<dl>
								...
								<dd>
									...
									<label for="bm_address" generated="true" class="error"><?=$WsbmModel->hasErrors('bm_address')?$form->error($WsbmModel,'bm_address'):"<strong>*</strong><span>用于寄发相关的材料，非常重要</span>"?></label>
								</dd>
							</dl>
							...
							<dl>
								...
								<dd>
									...
									<label for="bm_email" generated="true" class="error"><?=$WsbmModel->hasErrors('bm_email')?$form->error($WsbmModel,'bm_email'):"<strong>*</strong><span>请输入您最常使用的电子邮件地址</span>"?></label>
								</dd>
							</dl>
...
function changeNext(obj){
		var schoolid="<?=$_GET['id']?>";
        var kid = $(obj).val();
        var html = "<option value>--请选择--</option>";
        if(!kid){
            $("#bm_zycaption").html(html);//删除后面所有的选择。
        }else{
            $.ajax({
                url: "/education/getlistbyskid",
                type: "GET",
                data: {"kid":kid,"sid":schoolid},
                async: false,
                success: function(msg){
					...
					if(msg)html="";
                    for(var i in msg){
                        html += "<option value='"+i+"'>"+msg[i]+"</option>";
                    }
                    $("#bm_zycaption").html(html);
                }
            });
        }
    }
