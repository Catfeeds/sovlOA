...
<div class="new_right" style="border:1px solid">
	<div class="new_right_box">
		<div class="new_right_box_title">
			<ul>
				...
				<a href="<?=Yii::app()->createUrl("research/newslist",array("id"=>$NclassModel->ic_id))?>"><?=Icolumn::model()->getNameByid($NclassModel->ic_id)?></a></dd>
				 >
				<?php if($newsModel->i_label&&Icolumn::model()->getNameByid($newsModel->i_label)){?>
					<a href="<?=Yii::app()->createUrl("research/newslist",array("id"=>$newsModel->i_label))?>"><?=Icolumn::model()->getNameByid($newsModel->i_label)?></a></dd>
					>
				<?php }?>
				<strong>详细</strong></li>
			</ul>
