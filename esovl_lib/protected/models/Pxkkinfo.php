...
class Pxkkinfo extends CActiveRecord
{
	...
	public function getAllByid($id,$order='pxk_id desc',$pcl,$limit='10')
	{
		$criteria=new CDbCriteria;
		$criteria->addCondition("pxk_sid ='{$id}'");
		$criteria->order=$order;
		if($pcl){
			$criteria->addCondition("pxk_cl ='{$pcl}'");
		}
		$criteria->limit=$limit;
		$Info=$this->findAll($criteria);
		return $Info;
	}
	public function getAllByinfo($order='pxk_date desc',$limit='10')
	{
		$criteria=new CDbCriteria;
		$criteria->order=$order;
		$criteria->limit=$limit;
