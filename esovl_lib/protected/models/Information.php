...
class Information extends CActiveRecord
{
	...
	//获取所有拥有图片的新闻
	public function getAllHasImg($id='',$limit='10',$order='i_submitdate desc',$ishot=false,$hasImg=true)
	{
		$criteria=new CDbCriteria;
		if($id)$criteria->addCondition("i_class ='{$id}'");
		if($ishot)$criteria->addCondition(" i_bool = 1 ");
		if($hasImg)$criteria->addCondition(" i_pic != '' ");
	...
	//根据新闻父栏目ID得到新闻
	public function getAllByPid($pid='',$limit='10',$order='i_submitdate desc',$ishot=false,$offset=0)
	{
		$criteria=new CDbCriteria;
		if($pid){
	...
	//获取多条新闻
	public function getNeedById($id,$nid = array(),$limit = '',$order='i_submitdate desc',$iimg = false){
		$criteria = new CDbCriteria;
		$criteria->addCondition("i_class = '{$id}'");
		if(!empty($nid))$criteria->addCondition("i_id not in(".join(",",$nid).")");
		if($iimg)$criteria->addCondition(" i_pic <> ''");
		$criteria->limit=$limit;
		$criteria->order=$order;
