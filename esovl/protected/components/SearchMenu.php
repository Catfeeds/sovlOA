...
class SearchMenu extends CPortlet {
    ...
    private static $templeteKey = array(
            ...
            "section"=>"c",//版块
            "company"=>"d",//公司名称
			"characteristic"=>"t",//公司名称
            "level"=>"e",//等级
            "order"=>"f",//排序
			"jiguan"=>"j",//籍贯
			"age"=>"g",//年龄
    );
	public static $agearray = array(
            "0018"=>"18以下",
			"1820"=>"18-20",
			"2123"=>"21-23",
			"2426"=>"24-26",
     ...
     * 显示特色
     */
    private function showCharacteristic($data,$templeteKey) {
        echo "<p>";
        $this->getBuXianUrl($templeteKey);
        foreach($data as $key=>$value) {
            if(isset($this->searchCondition[$templeteKey])&&in_array($key,explode("9090",$this->searchCondition[$templeteKey]))) {
				$value.='<input type="checkbox" checked="checked" onclick=\'window.location.href=$(this).parents("a").attr("href")\'/>';
				$arr=explode("9090",$this->searchCondition[$templeteKey]);
				$arr1=array();
				foreach($arr as $k=>$v){
					if($v==$key)continue;
					$arr1[]=$v;
				...
				$key1=join("9090",$arr1);
				$this->getConditionUrl($value, $key1, $templeteKey);
				
            }else {
				$value.='<input type="checkbox" onclick=\'window.location.href=$(this).parents("a").attr("href")\'/>';
				$arr=array();
				if(isset($this->searchCondition[$templeteKey])&&count($this->searchCondition[$templeteKey])>=1){
					$arr=explode("9090",$this->searchCondition[$templeteKey]);
				}
				$arr[]=$key;
				$key1=join("9090",$arr);
	...
	private function showAge(){
        $data = SearchMenu::$agearray;
        $this->baseShow($data, "age");
    }
	private function showTese() {
		$data = Membercharacteristic::model()->getCharacteristic();
		if($data)
		$this->showCharacteristic($data, "characteristic");
	}
    /**
     * 显示区
     */
