...
<div class="main">
    ...
    <div class="px_pd_box03">
        ...
        <div class="px_pd_box03_center" id="px_pd_box03_center_01">
			...
			if($row != ''){?>
            	<dl>
                  <dt id="new01"><a href="px_news_list_details.php?id=<?=$row["nid"]?>" title="<?=$row["ntitle"]?>"><?=$row["ntitle"]?></a></dt>
            ...
            <div class="px_pd_box03_center_box02">
            	<ul>
                	  <?php
			$res = $dblink->findAll('ennews','',' nclass in(66,67,73,74)','','1,10');
			foreach($res as $row){?>
	<li><a href="px_news_list_details.php?id=<?=$row["nid"]?>" title="<?=$row["ntitle"]?>"><?php if (strlen($row["ntitle"])>35){echo $Common->strCut($row["ntitle"],35,'...');}else{echo $row["ntitle"];}?></a></li>
        ...
        <div class="px_pd_box03_right" id="px_pd_box03_right_01">
            ...
            <div class="px_pd_box03_right_list">
            	<ul>
                 <?php
						$sql="select * from xl_ask where w_downcl='px' and w_pindao='".$pdao."' limit 8";
    	...
    	<div class="mian_px_index_news_left">
            ...
            <div class="mian_px_index_ky_bk">
                ...
                <div class="mian_px_index_ky_news_bt">
                	<ul>          
           <?php
		  $res1 = $dblink->findAll('ennews','','smclass='.$row1["ps_id"],'','4');
          foreach($res1 as $row){?>
	...
	<li style="line-height:25px;"><a href="px_news_list_details.php?id=<?=$row["nid"]?>" title="<?=$row["ntitle"]?>"><?php if (strlen($row["ntitle"])>24){echo $Common->strCut($row["ntitle"],24,'...');}else{echo $row["ntitle"];}?></a><span><?=date("Y-m-d",strtotime($row["ndate"]))?></span></li>
          <?php }?>  
            ...
            <div class="mian_px_index_wyxw_bk" style="height:auto;">
            	<div class="mian_px_index_wyxw_bj">
                	<div class="mian_px_index_ywxw_bt"><span>在线问答</span><a href="px_wd_list.php">更多...</a></div>
                </div>
