      ...
      <div class="main_xllb">
        ...
        <div class="wl-zx-lj">
        		<dl>
                    ...
                    <dd><?=$n_class?></dd><dt><img src="<?=$z_website?>images/bx_hw_03.jpg" width="17" height="30" /></dt>
                </dl>
        </div>
        </div>
        <div class="wl-wzgg">
        	<div class="wl-gg-lm">
            	<div class="wl-gg-zl">网站公告</div>
            </div>
            <div class="wl-lm-dian"><img src="<?=$z_website?>images/wl-gg_06.jpg" /></div>
            <div class="wl-xw-dk">
            <?php
			$sql="select * from ennews where nclass='$classid' and nid!='$wl_id'  order by ndate desc";
			$rs=mysql_query($sql,$conn);
			$row=mysql_fetch_assoc($rs);
			$wl_id=$row['nid'];
			?>
            	<div class="wl-xw-tu"><a href="Education/wlys/wl_zx-neiye.php?nid=<?=$row['nid']?>"><img src="<?=$z_website?>admin_root/<?php echo $row['npic']?>" /></a></div>
                <div class="wl-xw-bt"><a href="Education/wlys/wl_zx-neiye.php?nid=<?=$row['nid']?>"><?php echo $row['ntitle']?></a></div>
                <div class="wl-xw-sj"><?php echo date('Y-m-d',strtotime($row['ndate']))?></div>
                <div class="wl-xw-nr"><?php echo $row['ncon']?></div>
            </div>
            <?php			
			$numq=mysql_query("select * from ennews where nclass='$classid' and nid!='$wl_id'");
			...
			$sql="select * from ennews where nclass='$classid' and nid!='$wl_id' limit $page,$PageSize";
			$rs=mysql_query($sql,$conn);
			while($row=mysql_fetch_assoc($rs)){
			?>
            <div class="wl-xw-dk2">
            	<div class="wl-wzgg-bt"><span><a href="Education/wlys/wl_zx-neiye.php?nid=<?=$row['nid']?>"><?php echo $row['ntitle']?></a></span><?php echo date('Y-m-d',strtotime($row['ndate']))?></div>
                <div class="wl-wzgg-nr"><?php echo $row['ncon']?><span><a href="Education/wlys/wl_zx-neiye.php?nid=<?=$row['nid']?>">查看详细>></a></span></div>
            </div>
            <?php }?>
            
            
            <div class="wl-gg-fy">
            	<div class="fy-syy"><a href="Education/wlys/wl_zx.php?classid=<?php echo $classid;?>&page=<?php echo ($CurrentPageID-1);?>"><img src="<?=$z_website?>images/wl-gg_18.jpg" /></a></div>
                <?php
				if($CurrentPageID>1){
					for ($k=($CurrentPageID-1);$k<$CurrentPageID&$k>0;$k++){
						echo '<div class="zx_fy_sz"><a href="Education/wlys/wl_zx.php?classid='.$classid.'&page='.$k.'">'.$k.'</a></div>';
					}
				}
				?>
                <div class="fy-sz2"><?php echo $CurrentPageID?></div>
                <?php
				if($CurrentPageID<$sumpage){
					for ($m=$CurrentPageID;$m<$sumpage&$m<($CurrentPageID+1);$m++){
						echo '<div class="zx_fy_sz"><a href="Education/wlys/wl_zx.php?classid='.$classid.'&page='.($m+1).'">'.($m+1).'</a></div>';
					}
				}
				?>
                <div class="fy-syy"><a href="Education/wlys/wl_zx.php?classid=<?php echo $classid;?>&page=<?php if($CurrentPageID<$sumpage) echo ($CurrentPageID+1); else echo $CurrentPageID;?>"><img src="<?=$z_website?>images/wl-gg_28.jpg" /></a></div>
            </div>
        </div>
      </div>
    </div>
