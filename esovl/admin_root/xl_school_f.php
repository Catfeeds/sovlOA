...
if (@$_COOKIE["a_class"]>1) {
  ...
  if (isset($_POST["kcedit"])){
      $sql="update xl_s_f set s_h_title='".$_POST["s_h_title"]."', s_h_pic='".$_POST["s_h_pic"]."', s_h_http='".$_POST["s_h_http"]."' where s_h_id=".$_POST["s_h_id"];
	  $rs=mysql_query($sql,$conn);
	  if ($rs){
	 
      echo"<script>alert('更新成功');location.href='xl_school_f.php?id=".$_POST["cid"]."';</script>";	  
	  }else{
  ...
  if (isset($_POST["zyadd"])){
      $sql="insert into xl_s_f set s_h_title='".$_POST["s_h_title"]."', s_h_pic='".$_POST["s_h_pic"]."', s_h_http='".$_POST["s_h_http"]."',classid=".$_POST["cid"];
	  $rs=mysql_query($sql,$conn);
	  if ($rs){	 
      echo"<script>alert('添加成功');location.href='xl_school_f.php?id=".$_POST["cid"]."';</script>";
	  }else{	  
...
<table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#93B3E9">
  <tr>
	...
	<?php
	  $sql="select * from xl_s_f where classid=".$_GET["id"];
	  $rs=mysql_query($sql,$conn);
	  if (!$rs){	 
    ...
    <td width="463" bgcolor="#FFFFFF" class="title"><input type="submit" name="Submit" onClick="if(document.cform<?php echo $i;?>.s_h_title.value==''){alert('标题为空');document.cform<?php echo $i;?>.s_h_title.focus();return false;}" value="更新"></td>
	  </tr>
  </form>
  <?php
			 }
...
<form name="formclassh" method="post" action="">
  <tr>
    <td colspan="7" class="mian_right_box_title_01">【添加方形广告】(3幅以内,前台只能显示3幅)</td>
	</tr>
	  <tr>
    <td width="126" height="33" align="right" bgcolor="#FFFFFF" class="title">新增类别标题：</td>
	<td width="160" colspan="-2" bgcolor="#FFFFFF" class="title"><input name="zyadd" value="kcok" type="hidden"><input name="cid" value="<?php echo $_GET["id"]?>" type="hidden">
    ...
    <td width="443" bgcolor="#FFFFFF" class="title"><input type="text" name="s_h_pic" style="width:150px;" readonly/>
    ...
    <td width="143" bgcolor="#FFFFFF" class="title"><input type="text" name="s_h_http"></td>
    <td width="232" bgcolor="#FFFFFF" class="title">
	  <input type="submit" onClick="if(document.formclassh.s_h_title.value==''){alert('标题为空');document.formclassh.s_h_title.focus();return false;}" name="Submit" value="添加"></td>
    </tr>
  </form>
</table>
</div>
