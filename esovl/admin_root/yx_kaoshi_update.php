...
if (isset($_POST["kaoshi_title"])){
   $sql="update yx_kaoshi set kaoshi_title='".$_POST["kaoshi_title"]."',class_pindao='".$_POST["class_pindao"]."',kaoshi_con='".$_POST["kaoshi_con"]."',pxk_pic='".$_POST["pxk_pic"]."' where kaoshi_id=".$_GET["id"];
	  $rs=mysql_query($sql,$conn);
	  if ($rs){	
      echo"<script>alert('修改成功');location.href='yx_kaoshi_list.php';</script>";
	  }else{	  
	  exit("更新失败! 错误信息为:".mysql_error());
}
}
$sql="select * from yx_kaoshi where kaoshi_id=".$_GET["id"];
$rs=mysql_query($sql,$conn);
if (!$rs){	 
...
<div class="s_info">
  <table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#93B3E9">
    <form name="pxkkform" method="post" onSubmit="return pxkaike()" action="">
	  ...
	  <?php 
      $sql2="select * from yx_kaoshi where kaoshi_id=".$_GET["id"];
	  ...
	  <tr>
	    <td height="38" align="right" bgcolor="#FFFFFF" class="title">考试名称：</td>
	    <td width="919" colspan="-2" bgcolor="#FFFFFF" class="title">
        <input type="text" name="kaoshi_title" id="pxk_title" value="<?=$row["kaoshi_title"]?>" maxlength="100" style="width:300px;"/><span id="p_kbmc">*</span></td>
      </tr>
